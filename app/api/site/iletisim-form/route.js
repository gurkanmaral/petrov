...
export async function POST(request) {
  ...
  const { name, surname, phone, email, subject, message } = res;
  const { db } = await connectToDatabase();

  const result = await db.collection("contact-forms").insertOne({
    name,
    surname,
    phone,
    email,
...
export async function GET() {
  const { db } = await connectToDatabase();
  const result = await db.collection("contact-forms").find({}).toArray();
  return NextResponse.json({
    result,
  });
}
